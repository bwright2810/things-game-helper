...
var thingsapp_1 = __webpack_require__(332);
var $ = __webpack_require__(124);
window.things = new thingsapp_1.ThingsApp();
$("#game-id").keyup(function (event) {
    if (event.keyCode == 13) {
        $("#join-btn").click();
    }
});
...
Object.defineProperty(exports, "__esModule", { value: true });
var $ = __webpack_require__(124);
var izitoast = __webpack_require__(333);

var ThingsApp = function () {
    function ThingsApp() {
        var _this = this;

        _classCallCheck(this, ThingsApp);

        this.openWs = function () {
            _this.socket = new WebSocket('ws://localhost:4567/echo');
        ...
        this.reopenWsIfNeeded = function () {
            if (_this.socket.readyState != 1) {
                _this.openWs();
            }
        };
        this.joinGame = function () {
            var error = _this.validate(true);
            if (error != null) {
                _this.errorMsg(error);
                return;
            }
            var textField = document.getElementById("game-id");
            var text = textField.value;
            console.log("Sending: " + text);
            _this.socket.send(text);
        };
        this.newGame = function () {
            var error = _this.validate(false);
            if (error != null) {
                _this.errorMsg(error);
                return;
            }
            var nick = _this.inputValue($('#name').get(0));
            $.post('/create', { nickname: nick }).then(function (res) {
        ...
        this.validate = function (joining) {
            var nick = _this.inputValue($('#name').get(0));
            if (nick == "") {
                return "Please enter a nickname";
            }
            if (joining) {
                var gameId = _this.inputValue($('#game-id').get(0));
                if (gameId == "") {
    ...
    _createClass(ThingsApp, [{
        key: "inputValue",
        value: function inputValue(element) {
